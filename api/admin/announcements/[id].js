...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'PUT') {
    ...
    try {
      const { data, error } = await supabase.from('ANNOUNCEMENT').update({ AnnounceTitle: title, AnnounceContent: content, AnnounceType: type || 'General', AnnounceDateUpdated: new Date().toISOString() }).eq('AnnounceID', id).select().single();
