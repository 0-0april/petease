...
module.exports = async function handler(req, res) {
  ...
  try {
    const { data, error } = await supabase.from('ANNOUNCEMENT').update({ AnnounceTitle: title, AnnounceContent: content, AnnounceType: type || 'General', AnnouncedBy: user.accId, AnnounceDateUpdated: new Date().toISOString() }).eq('AnnounceID', id).select().single();
