...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const { data, error } = await supabase.from('ADOPTION').select(`
      ...
      USERPETS!ADOPTION_UserPetsID_fkey ( PetID, UserPetID, USER ( UserID, UserName ), PET ( PetName, PetImg, PetBreed ) )
    `).eq('UserID', userData.UserID).order('AdoptReqDate', { ascending: false });
    if (error) throw error;

    res.json((data || []).map(a => ({
      ...a, PetName: a.USERPETS?.PET?.PetName, PetImg: a.USERPETS?.PET?.PetImg, PetBreed: a.USERPETS?.PET?.PetBreed,
      PetID: a.USERPETS?.PetID, OwnerUserID: a.USERPETS?.USER?.UserID, owner_name: a.USERPETS?.USER?.UserName,
    })));
  } catch (error) {
