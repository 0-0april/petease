...
module.exports = async function handler(req, res) {
  ...
  const { id } = req.query;
  try {
    const { data, error } = await supabase.from('ADOPTION').update({ AdoptStatus: 'Completed' }).eq('AdoptID', id)
    ...
    if (data.USER?.AccID) {
      await createNotification({ accId: data.USER.AccID, title: 'Adoption completed', message: `The adoption process for ${petName} was completed.`, type: 'adoption' });
    }
    res.json(data);
  } catch (error) {
